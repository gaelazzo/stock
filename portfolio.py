...
def saveClosedPositions():    
    with open('data/closedpositions.csv', 'w',newline="") as csvfile:
        fieldnames = ['name', 'reuters','bloomberg','buyprice','q','buydate','sellprice','selldate','totcomm','tax','credits',"buycomm","sellcomm"]
...
def evaluateCommissions():
    operations = []
    for openPos in openPositions:         
         operations.append({"kind":"B","date":openPos["buydate"],"reuters":openPos["reuters"], "price":float(openPos["buyprice"]), "q":int(openPos["q"]),"row":openPos})       
    
    for closedPos in closedPositions:
         operations.append({"kind":"B","date":closedPos["buydate"],"reuters":closedPos["reuters"],"price":float(closedPos["buyprice"]),"q":int(closedPos["q"]),"row":closedPos})       
         operations.append({"kind":"S","date":closedPos["selldate"],"reuters":closedPos["reuters"],"price":float(closedPos["sellprice"]),"q":int(closedPos["q"]),"row":closedPos})       
    ...
    for op in operations:
        if ("evaluated" in op):
            continue
        opSet = [o for o in operations if o["kind"]==op["kind"] and o["date"]==op["date"] and o["price"]==op["price"] and o["reuters"]==op["reuters"]]
...
def evaluatePortfolio():
    ...
    for openPos in openPositions:
        ...
        stock["totPrice"]=float(stock["totPrice"])+float(openPos["buyprice"])*float(openPos["q"]) ##total buy price
...
def getTotals(opList=None):
    ...
    DetailedTotals["net"]= round(DetailedTotals["closedPL"]- DetailedTotals["tobin"]-DetailedTotals["comm"]-DetailedTotals["tax"],2)
    return DetailedTotals
