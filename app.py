...
def getGraphData():    
    ...
    fig.add_trace(
         go.Scatter(
             x=df["prettyDate"],
             y=df["BBup"],
             line = dict(color = "rgba(229,229,242,0.1)"), #,0.05
             name = "BB band",       
             fill= "tonexty",
             legendgroup="BB", 
             showlegend=False,
    ...
    fig.add_trace(
        go.Candlestick(
             ...
             #increasing_fill_color= 'rgb(0,202,115)', 
             increasing_line_color='rgb(0,202,115)', 
             decreasing_line_color= 'rgb(255,105,96)',
             #decreasing_fill_color= 'rgb(255,105,96)',
    ...
    print("apply lambda activeBullishPatterns")
    activeBullishPatterns = df[df.apply(lambda x: len(x["bullishPatterns"]) !=0, axis=1)]

    if (len(activeBullishPatterns)>0):
         print("there are activeBullishPatterns")
         activeBullishPatterns.loc[:, "patternDescription"] = ""
         print("activeBullishPatterns assigned")
         _min = df["Low"].min()
         _max = df["High"].max()
         _displace = (_max-_min)/20
         for i in range(len(activeBullishPatterns)):
                 ...
                 print(f"activeBullishPatterns.loc[{idx}]")
         print("after for range activeBullishPatterns")
         #print(activeBullishPatterns['High'])
         #print(_displace)
         fig.add_trace(go.Scatter(
                           x=activeBullishPatterns["prettyDate"],
                           y=activeBullishPatterns['High'] - _displace,
                           mode="markers",
                           #size="DivSize",
                           #symbol= "D",
                           #text = divs["divs"],
               ...
               row=1, col=1)
         print("after for add_trace activeBullishPatterns")

    activeBearishPatterns = df[df.apply(lambda x: len(x["bearishPatterns"]) !=0, axis=1)]
    if (len(activeBearishPatterns)>0):
         print("there are activeBearishPatterns")
         activeBearishPatterns.assign(patternDescription="")
         print("activeBearishPatterns assigned")
         _min = df["Low"].min()
         _max = df["High"].max()
         _displace = (_max-_min)/20
         for i in range(len(activeBearishPatterns)):
                 ...
                 print(f"activeBearishPatterns.loc[{idx}]")
         print("after for range activeBearishPatterns")
         aa = activeBearishPatterns['Low'] - _displace
         print("after assignment simulation")
         fig.add_trace(go.Scatter(
                           x=activeBearishPatterns["prettyDate"], 
                           y=activeBearishPatterns['Low']-_displace,
                           mode="markers",
                           #size="DivSize",
                           #symbol= "D",
                           #text = divs["divs"],
               ...
               row=1, col=1)
         print("after for add_trace activeBearishPatterns")
...
def graph():
    ...
    period = request.args.get("period","null")
    graph=None;
    if (bloomberg != "" and freq!="null" and period!="null"):
        graph = stockval.getGraph(bloomberg, period, freq)
...
def portfolioValues():    
    ...
    rend = {}
    for s in stocksData:
        divYeld= float(s["dividendYield"]) if s["dividendYield"] != "n/d" else None
        div= float(s["dividend"]) if s["dividend"] != "n/d" else None
        rend[s["reuters"]]={"divYeld": divYeld, 
                            "div":div}
    for s in portfolio:
...
def addPosition():
    ...
    msg= pf.addOpenPosition(request.form.get('reuters').strip().upper(),
...
def closePosition():
    ...
    #addOpenPosition(code,price,q,acqdate,acqcomm
    msg= pf.closePosition(request.form.get('reutersSellCode').strip().upper(),
